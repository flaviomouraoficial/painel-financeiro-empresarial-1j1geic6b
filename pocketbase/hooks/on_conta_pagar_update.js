onRecordAfterUpdateSuccess((e) => {
  ...
  if (statusAtual === 'paga' && statusOriginal !== 'paga') {
    let exists = false
    try {
      $app.findFirstRecordByData('lancamentos', 'conta_pagar_id', e.record.id)
      exists = true
    } catch (_) {}
    ...
    if (!exists) {
      ...
      lancamento.set('conta_pagar_id', e.record.id)

      if (e.record.getString('fornecedor_id'))
        lancamento.set('fornecedor_id', e.record.getString('fornecedor_id'))
      if (e.record.getString('projeto_id'))
        lancamento.set('projeto_id', e.record.getString('projeto_id'))
      if (e.record.getString('centro_custo_id'))
        lancamento.set('centro_custo_id', e.record.getString('centro_custo_id'))

      lancamento.set(
        'descricao',
        `Pagamento: ${e.record.getString('descricao') || e.record.getString('numero_nf') || 'Conta a Pagar'}`,
      )
      lancamento.set('valor', e.record.getFloat('valor_total'))
      lancamento.set(
        'data_lancamento',
        e.record.getString('data_pagamento') ||
